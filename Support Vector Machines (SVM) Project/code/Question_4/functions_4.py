...
def OAA(X_train, Y_train, ref_y_label):    
    Y_train = np.where(Y_train != ref_y_label, -1, Y_train) #convert other labels in -1
    Y_train = np.where(Y_train == ref_y_label, 1, Y_train) #convert the label ref_y_label in 1

    return X_train, Y_train

def OAO(X_train_all, Y_train_all, ref_y_label1, ref_y_label2):
    X_train=X_train_all[np.where((Y_train_all == ref_y_label1) | (Y_train_all == ref_y_label2))[0],:] #filtro X_train solo per le classi che sto considerando
    Y_train=Y_train_all[np.where((Y_train_all == ref_y_label1) | (Y_train_all == ref_y_label2))[0]] # faccio lo stesso per Y_train
    Y_train = np.where(Y_train == ref_y_label1, -1, Y_train) #convert the label ref_y_label1 in -1 
    Y_train = np.where(Y_train == ref_y_label2, 1, Y_train) #convert other labels in 1
...
def decomposition_algorithm_MVP(X_train, Y_train, gamma, C):
    ...
    while m > M+10**-8:
        ...
        #MEMORIZZO LE RIGHE DI Q IN UN DIZIONARIO: 
    ...
    return alpha, number_of_Q_rows_computed, k, KKT_violation, dual_objective(rbf_kernel(X_train,X_train, gamma), Y_train, alpha)

def dual_objective(K_train, Y_train, alpha): 
    Y_diag = np.diag(Y_train)
    Q = np.dot(np.dot(Y_diag,K_train), Y_diag).astype("float") 
    e = np.ones(len(Y_train)).astype("float") 
    return  0.5*np.dot(np.dot(alpha.T, Q),alpha) - np.dot(e.T,alpha)


def predict_OAA(X_train, Y_train, K_test, alpha, gamma, C):
    indici_SV = np.where((alpha > 1e-7) & (alpha < C - 1e-7))[0]
    if len(indici_SV) == 0:
        raise ValueError("Nessun SV con 0 < alpha < C.")
    #indici_BSV = np.where((alpha >= C - 1e-7 ) & (alpha <= C + 1e-7))[0]
    #print("# BSV / # totale sample = ", indici_BSV.shape[0], "/", alpha.shape[0])
    K_sv = rbf_kernel(X_train[indici_SV,:],X_train, gamma) # K_sv contiene solo le colonne di K associate a indici_SV  # K_sv contiene solo le colonne di K associate a indici_SV
...
def voting(column): #restituisce l'elemento più frequente in una colonna
    voted_pred = np.argmax(np.bincount(column.astype(int))) #bincount restituisce il numero di occorrenze per ciascun elemento nella colonna
    return voted_pred

def predict(Y_train,K_train, K_test, alpha, C):
    indici_SV = np.where((alpha > 1e-5) & (alpha < C - 1e-5))[0]
    #if len(indici_SV) == 0:
    #    raise ValueError("Nessun SV con 0 < alpha < C.")
    #indici_BSV = np.where((alpha >= C - 1e-5 ) & (alpha <= C + 1e-8))[0]
    #print("# BSV / # totale sample = ", indici_BSV.shape[0], "/", alpha.shape[0])
...
def print_confusion_matrix(y_test, Y_test):
    ...
    # Visualizzazione della matrice di confusione per 3 classi
